...
不要把从 skills.sh、gstack、clawhub 或其它来源安装的外部 skill 放进 `my-skills/`；三方或导入型 skill 放 `skills/`，分配关系放 profile，运行副本放 `.agents/skills/` 和 `.claude/skills/`。
...
如果 Codex/Agent 中出现重复自研 skill，只保留 `my-skills/<skill-name>/` 发布源，并通过 profile 决定是否生成根或子项目运行副本。不要把外部安装包、symlink 或运行副本放进 `my-skills/`。
