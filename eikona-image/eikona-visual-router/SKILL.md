...
先判断 provider 是否可用，再判断视觉请求的 owner、用途和证据链，并加载最小的 Eikona workflow/director skill。路由器不直接包办最终 prompt 或图像生成。
...
| 提供 base URL、API key、channel、模型 ID，要求安装/配置/调通 Eikona | `eikona-gateway-bootstrap` | 用户级安全接入；key 只走 stdin，不进入参数或项目文件；Nano Banana 使用网关准确 ID。 |
...
7. 本地验证默认用 `fixture:image`；真实远程生成默认用 `openai:gpt-image-2`，其他模型使用网关准确 model ID。
...
eikona generate --model openai:gpt-image-2 --aspect 3:1 --size 2k --input prompts/story/storyboard/scene/prompts/01-planning-board.md --json
...
eikona generate --model openai:gpt-image-2 --input prompts/product/landing-hero/launch/prompts/01-clean-editorial.md --size 2k --json
```
...
- 不把 key 放入命令参数、项目文件、日志或结构化输出；配置凭据必须使用默认用户级 `eikona auth set --api-key-stdin` 流程。
