...
5. For agent invocation design, keep the contract simple:
   ...
   - visual scoring uses `eikona assess <run_id> --fixture --agent` (deterministic fixture channel, marked `channel=fixture`/`test_only`); without `--fixture` the real channel fails closed typed (`MODEL_UNCONFIGURED` until a vision scorer is wired — never a silent fixture fallback) and real results carry `channel=real` with model ref/version and explicitly indeterminate missing dimensions; scores and tags remain review evidence, while acceptance still requires append-only human feedback;
