...
1. Product repo, homebrew tap, and scoop bucket stay private. Only deliberately public derivative repos (for example `yeisme-agent-my-skills`) may be public, and release-time checkouts must never depend on the private monorepo.
2. Installs authenticate: `<PRODUCT>_RELEASE_TOKEN` first, then `gh auth token`. The bearer token is attached only to `github.com` / `api.github.com` hosts — never to test endpoints or third-party domains.
...
4. **Verify**: download assets + `sha256sum --check checksums.txt --ignore-missing`, `--version`, installer plan/apply/doctor with a token, fixture smoke.
5. **On failure**: capture the failing step, clean the burnt tag (draft + tag + channel manifests), fix on develop, repeat with the next patch version.
...
- Channel matrix, onboarding checklist, and measured pitfalls: root `docs/operations/private-release-reference.md`.
