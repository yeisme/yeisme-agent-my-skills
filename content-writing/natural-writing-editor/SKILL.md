...
| 触发 | 一线修复 | 仍失败 |
| --- | --- | --- |
| 改写改了数字、责任或限定 | 对照保真清单回滚该处 | 找不到更好版本就保留原文 |
| 用检测器分数当质量门 | 停；本 skill 不承诺“无法检测” | 不以分数验收 |
| 叠加 Humanizer / de-ai-writing 全文再改一遍 | 默认只跑本流程一次 | 仅用户点名才加载社区 skill |
| 无样稿却发明作者个性 | 只保留原文已有习惯 | 不复制标志性短语、不推断私人属性 |
| 为“像人”加口语/错别字/随机长短句 | 删掉补丁 | 宁可保留可接受原句 |
