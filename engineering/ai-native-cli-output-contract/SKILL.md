...
For Pi, OMP, Cohors, Auctra, pinax, and similar agent tools, CLI help text, default command summaries, logs, user-visible errors, and `--explain` reports default to English. Human-authored project docs, OpenSpec plans, reviews, handoffs, and run summaries default to Chinese unless a subproject explicitly marks the artifact as public English documentation. Machine protocol fields, schema keys, enum values, command names, flags, log keys, JSON envelope fields, `--agent` keys, and third-party API fields remain stable English or existing names.
