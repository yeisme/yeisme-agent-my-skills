...
- `--explain`: human-readable conclusion, evidence, confidence, and next action.
...
- Do not include ANSI, tables, prose paragraphs, debug dumps, or localized section labels.
...
- Use `结论`, `证据`, `置信度`, and optional `推荐下一步`.
- Every conclusion must have evidence or be explicitly marked as a hypothesis.
- Do not expose raw prompts, secrets, credentials, cookies, tokens, or unredacted provider payloads.
...
3. Implement renderers from the projection: human summary, agent, JSON, events, and explain.
...
5. Add tests close to the command:
...
6. Update help text and docs with real user-runnable commands only. Do not mention local execution wrappers, shell aliases, or agent-only prefixes.
...
```bash
python my-skills/ai-native-cli-output-contract/scripts/validate_cli_output.py --mode json < output.json
python my-skills/ai-native-cli-output-contract/scripts/validate_cli_output.py --mode agent < output.agent
python my-skills/ai-native-cli-output-contract/scripts/validate_cli_output.py --mode events < output.ndjson
```

For live commands, use real CLI examples:

```bash
app task get T123 --json | python my-skills/ai-native-cli-output-contract/scripts/validate_cli_output.py --mode json
app task get T123 --agent | python my-skills/ai-native-cli-output-contract/scripts/validate_cli_output.py --mode agent
app task watch T123 --events | python my-skills/ai-native-cli-output-contract/scripts/validate_cli_output.py --mode events
```
...
- Do not emit secrets, auth headers, provider tokens, raw prompts, cookies, or full unredacted stack traces in any output mode.
