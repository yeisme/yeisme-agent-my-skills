...
- Private service: `internal/serviceauth` owns the CLI-authored service config and scoped access-key store (salted-hash-only persistence, 0600 secret files). `eikona service config/access-key/doctor` author them; `eikona serve --config` mounts the TLS/trusted-proxy guard (`TLS_REQUIRED` before any side effect) and access-key authentication. `GET /api/v1/readiness` (`internal/api/runtimesurface`) projects ten independent readiness dimensions; fixture evidence must never promote live provider or public-hosting readiness.
- MCP: `eikona mcp --transport stdio` without machine-output flags runs the real JSON-RPC lifecycle (`internal/mcp/stdio.go`; stdout protocol-only, redacted stderr, stdin-EOF shutdown); `eikona mcp --json|--agent` stay one-shot projections; `eikona mcp doctor` is the diagnostic; remote stdio delegates to the official sibling adapter and fails `MCP_SIBLING_ADAPTER_UNAVAILABLE` instead of inventing a transport.
...
5. For agent invocation design, keep the contract simple:
   ...
   - visual scoring uses `eikona assess <run_id> --agent`; scores and tags remain review evidence, while acceptance still requires append-only human feedback;
