...
| Homebrew | Default to `homebrew_casks`, copying an existing maintained Go CLI `.goreleaser.yaml` pattern. Use legacy `brews`/Formula only when an existing project already relies on formula semantics. Do not add both cask and formula for the same binary without an explicit project decision. |
...
Every release-distribution change needs at least one archive checksum verification and one install smoke. Follow a maintained Go CLI release workflow pattern:
